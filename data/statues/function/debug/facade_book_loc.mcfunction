...
execute as @e[tag=facade_book_pool,type=minecraft:marker] at @s if data block ~ ~ ~ Items[{"components":{"custom_data":{facade_book_item:1b}}}] run tag @s add facade_book_haskey
execute as @e[tag=facade_book_pool,type=minecraft:marker] at @s if data block ~ ~ ~ {LootTable:"statues:items/facade/objectives/book_7"} run tag @s add facade_book_haskey
