...
execute as @a[tag=!dev_ignore_flashlight,tag=!marksman,team=!Monster] at @s if predicate statues:holding_flashlight if score $Lights Statues.DynamicData matches 1 run function statues:mechanics/flashlight_alt/use