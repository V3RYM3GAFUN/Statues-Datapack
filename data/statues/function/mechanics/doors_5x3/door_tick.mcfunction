...
execute at @s[y_rotation=0] align xyz positioned ~-2.5 ~ ~-1.5 if entity @a[dx=4,dy=2,dz=3,gamemode=!spectator,limit=1,sort=nearest] run tag @s add door5x3_opening
execute at @s[y_rotation=90] align xyz positioned ~-1.5 ~ ~-2.5 if entity @a[dx=3,dy=2,dz=4,gamemode=!spectator,limit=1,sort=nearest] run tag @s add door5x3_opening
