...
$item replace entity @s hotbar.8 with compass[custom_data={"is_compass_tracker":1b},item_name='{"translate":"statues.item.objective.tracker","italic":false}',lodestone_tracker={target:{pos:[$(compass_target_pos_x),$(compass_target_pos_y),$(compass_target_pos_z)],dimension:"minecraft:overworld"},tracked:true}]
