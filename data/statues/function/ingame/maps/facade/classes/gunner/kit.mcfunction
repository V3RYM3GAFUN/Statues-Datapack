...
item replace entity @s hotbar.4 with blaze_rod[use_cooldown={seconds:210},consumable={animation:bow,consume_seconds:0.01,has_consume_particles:false,sound:"item.armor.equip_netherite"},item_name='{"translate":"statues.item.gunner.ability.name","italic":false,"color":"dark_aqua"}',lore=['{"translate":"statues.item.cooldown.min","with": [{"text":"3"},{"text": "30"}],"italic":false}'],item_model="arrow",custom_data={"gunner_ability":true}]
