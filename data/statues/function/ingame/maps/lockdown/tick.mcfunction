...
execute if score $Timer.Ticks Statues.TempData matches 380 as @a at @s run playsound statues:game.lockdown.intro neutral @a ~ ~ ~ 1 1
...
execute if score $Timer.Ticks Statues.TempData matches 20.. as @a[team=Survivor,tag=class_selected] run attribute @s jump_strength modifier add statues:no_jump -1 add_value
execute if score $Timer.Ticks Statues.TempData matches ..19 as @a[team=Survivor,tag=class_selected] run attribute @s jump_strength modifier remove statues:no_jump
...
execute if score $Objective Statues.DynamicData matches 14 as @e[tag=objective_lockdown_escape,type=marker,x=0] at @s positioned ~-1 ~ ~-1 as @a[team=Survivor,dx=2,dy=1,dz=2] run function statues:ingame/maps/lockdown/objectives/escape
