...
execute at @e[tag=trap,type=marker,x=0,limit=4] unless entity @n[tag=trap_display,type=item_display,distance=..1] align xyz run summon item_display ~.5 ~ ~.5 {item:{id:breeze_rod,components:{custom_model_data:{strings:["trap"]}}},Tags:["trap_display"]} 
...
execute as @e[tag=trap_display,type=item_display,limit=4,x=0] at @s unless entity @n[tag=trap,type=marker,distance=..1] run kill @s
