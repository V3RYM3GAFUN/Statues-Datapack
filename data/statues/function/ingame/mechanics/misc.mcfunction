...
execute at @e[tag=trap,type=marker,limit=4] unless entity @e[tag=trap_display,type=item_display,distance=..1,limit=1] align xyz run summon item_display ~.5 ~ ~.5 {item:{id:breeze_rod},Tags:["trap_display"]}
...
execute as @e[tag=trap_display,type=item_display,limit=4] at @s unless entity @e[tag=trap,type=marker,distance=..1,limit=1] run kill @s
