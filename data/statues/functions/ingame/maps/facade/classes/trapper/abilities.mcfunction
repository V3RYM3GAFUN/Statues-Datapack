...
execute if score @s Statues.Players matches 1..4200 unless data entity @s Inventory[{Slot:4b,id:"minecraft:carrot_on_a_stick"}] run item replace entity @s hotbar.4 with carrot_on_a_stick{Unbreakable:1,CustomModelData:5,display:{Name:'[{"text":"EMP shock","italic":false,"color":"gray"}]',Lore:['[{"text":"Cooldown: 3m30s. Will also affect the user","italic":false}]']},HideFlags:4}
