...
execute as @e[type=minecraft:interaction,tag=hide] at @s on target run scoreboard players set @s[tag=!hidden,team=Survivor] Statues.Heartbeat 25
...
execute as @a[tag=hidden] at @s if score @s Statues.Heartbeat matches 0 run playsound entity.warden.heartbeat ambient @s ~ ~ ~ 5 0.5
execute as @a[tag=hidden] at @s if score @s Statues.Heartbeat matches 0 run scoreboard players set @s Statues.Heartbeat 25
