execute if entity @s[tag=!triggered] if entity @a[distance=..2,gamemode=!spectator] run scoreboard players set @s Statues.DynamicData 50
...
execute if score @s Statues.DynamicData matches -1 if entity @a[distance=2..,gamemode=!spectator] run tag @s remove triggered