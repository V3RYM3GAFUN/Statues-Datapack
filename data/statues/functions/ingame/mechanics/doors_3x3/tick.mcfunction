...
execute at @s[y_rotation=0] positioned ~-1 ~-1 ~-1.5 if entity @a[dx=3,dy=3,dz=3,gamemode=!spectator,limit=1,sort=nearest] run tag @s add door3x3_opening
execute at @s[y_rotation=90] positioned ~-1.5 ~-1 ~-1 if entity @a[dx=3,dy=3,dz=3,gamemode=!spectator,limit=1,sort=nearest] run tag @s add door3x3_opening
